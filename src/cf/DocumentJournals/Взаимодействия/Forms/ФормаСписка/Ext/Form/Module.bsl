...
Процедура ОтправитьПолучитьПочтуВыполнить(Команда)
	...
	УправлениеЭлектроннойПочтойКлиент.ОтправитьЗагрузитьПочтуПользователя(УникальныйИдентификатор, ЭтотОбъект, Элементы.Список);
...
Процедура ПриИзмененииСтатусаСервер(ОбновлятьПанельНавигации)
	...
	ПредставлениеСтатуса = Взаимодействия.СписокСтатусов().НайтиПоЗначению(Статус).Представление;
...
Процедура ИзменитьОтборСписок(ИмяТаблицы, ДанныеДляОбработки);
	...
	Если ИмяТаблицы = "Предметы" 
		Или ИмяТаблицы = "Контакты"
		Или ИмяТаблицы = "Закладки" Тогда
	...
	СтруктураСвойствСписка.ТекстЗапроса = ТекстЗапросаДинамическогоСписка;
	...
	Если ТолькоЗначимыеПредметы Тогда
		...
		Для Каждого СтрокаУсловия Из МассивСтрокУсловия Цикл
			...
			ИначеЕсли СтрНайти(СтрокаУсловия, "ПредметыВзаимодействий") Тогда
				Если ПустаяСтрока(ТекстУсловийПоРегистру) Тогда
					СтрокаУсловия = Прав(СтрокаУсловия, СтрДлина(СтрокаУсловия) - 3);
				КонецЕсли;
		...
		Если Не ПустаяСтрока(ТекстУсловийПоРегистру) Тогда
			ТекстУсловийПоРегистру = "ГДЕ" + " " + ТекстУсловийПоРегистру
		КонецЕсли;
...
Процедура СоздатьНовоеВзаимодействие(ТипОбъекта)
	...
	ВзаимодействияКлиент.СоздатьНовоеВзаимодействие(ТипОбъекта,, ЭтотОбъект);
