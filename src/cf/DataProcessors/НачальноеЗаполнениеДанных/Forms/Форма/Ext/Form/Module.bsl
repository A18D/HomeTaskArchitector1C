...
Процедура Подключаемый_ТабличныйДокументПриИзменении(Элемент)
	...
	ЭтоИзменениеОбласти = Не ЭтоИзменениеСтрок И Не ЭтоИзменениеКолонок И Не ЭтоИзменениеЯчейки;
	...
	Если ПустаяСтрока(ИмяТабличнойЧасти) Тогда
		Если ИмяРеквизита = "ИмяПредопределенныхДанных" Тогда
			...
			Если СтрНайти(ИменаПредопределенныхЗначений, Результат + ",") = 0 Тогда
				...
				Попытка
					новУникальныйИдентификатор = Новый УникальныйИдентификатор(Результат);
				Исключение
					// и это не ГУИД
...
Процедура ТабличныйДокументПриИзмененииТипизацияЗначений(ИмяТабличнойЧасти)
	...
	МенеджерОбъекта = ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(Объект.ОбъектМетаданных);
	...
	Для каждого СтрокаТаблицы Из ТаблицаДанныхОбласти Цикл
		...
		Если ПустаяСтрока(ИмяТабличнойЧасти) И ИмяРеквизита = "ИмяПредопределенныхДанных" Тогда
			Если СтрНайти(ИменаПредопределенныхЗначений, ТекстОбласти + ",") = 0 Тогда
				Попытка
					новУникальныйИдентификатор = Новый УникальныйИдентификатор(ТекстОбласти);
				Исключение
		...
		ИначеЕсли ЭтоСсылочныйТип(ТипРеквизитаСтруктуры) Тогда
			...
			ТекстОтбора = "";
			Для каждого СтандартныйРеквизит Из ЗначениеРеквизитаПоУмолчанию.Метаданные().СтандартныеРеквизиты Цикл
			...
			|ИЗ
			|	" + ЗначениеРеквизитаПоУмолчанию.Метаданные().ПолноеИмя()+" КАК ТаблицаИсточник
			|ГДЕ
			|	ЛОЖЬ" + ТекстОтбора);
...
Процедура ЗаполнитьДеревоРеквизитов()
	...
	Для каждого Реквизит Из ПолнаяСтруктураОбъекта Цикл
		...
		НоваяСтрока.Картинка          = ?(НоваяСтрока.Локализуемый, БиблиотекаКартинок.ГеографическаяСхема, БиблиотекаКартинок.Реквизит);
		...
		Для каждого РеквизитТЧ Из Реквизит.Значение.Реквизиты Цикл
			...
			НоваяСтрокаТЧ.Картинка          = ?(НоваяСтрокаТЧ.Локализуемый, БиблиотекаКартинок.ГеографическаяСхема, БиблиотекаКартинок.Реквизит);
...
Функция ПолучитьТекстЗапросаВыборкиДанных()
	...
	|	ОбъектыМД.*
	|ИЗ
	|	" + Объект.ОбъектМетаданных + " КАК ОбъектыМД
	|ГДЕ
	|	Истина";
...
Процедура ЗаполнитьТабличныйДокумент(ТабличныеДокументы, ИсточникДанных, ДанныеИнициализации = Неопределено, ИмяТабличнойЧасти = "", ИмяПредопределенныхДанных = "")
	...
	Для Каждого СтрокаДанных Из ИсточникДанных Цикл
		...
		ИмяПредопределенныхДанныхШапка = "";
		...
		Для Каждого Реквизит Из РеквизитыИнициализации Цикл
			...
			Если ПустаяСтрока(ИмяТабличнойЧасти) И Реквизит.Имя = "ИмяПредопределенныхДанных" Тогда
				ИмяПредопределенныхДанныхШапка = СтрокаДанных[Реквизит.Имя];
			КонецЕсли;
...
Процедура ЗаполнитьДинамическиеЭлементыФормы()
	...
	Для Каждого Реквизит Из ПолнаяСтруктураОбъекта Цикл
		...
		Элемент.УстановитьДействие("ПриАктивизацииОбласти", "Подключаемый_ТабличныйДокументПриАктивизацииОбласти");
...
Процедура ПреобразоватьВКодНаСервере()
	...
	БлокиКода1С = Новый Соответствие;
		...
		Для каждого Реквизит Из РеквизитыИнициализации Цикл
			...
			ИначеЕсли ЭтоПростойТип И ОписаниеТиповСтрока.СодержитТип(ТипРеквизита)
				...
				Если ИмяРеквизита <> "ИмяПредопределенныхДанных" Тогда
					...
					Если ИспользоватьНСтр Тогда
						ЗначениеРеквизитаСтрока = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку("НСтр(""%1 = '%2'"")", "ru", ЗначениеРеквизитаСтрока);
					Иначе
			...
			Иначе//Если ЭтоСсылочныйРеквизит Тогда
				...
				ИначеЕсли ОбщегоНазначения.ЭтоПеречисление(МетаданныеРеквизита) И НЕ ПустаяСтрока(ЗначениеРеквизита) Тогда
					...
					ИмяПеречисления = МетаданныеРеквизита.ЗначенияПеречисления[ИндексПеречисления].Имя;
...
Функция ПолучитьРезультатЗаполненияПоКоду()
	...
	ТекстКода1С = СтрЗаменить(ТекстКода1С, "МультиязычностьСервер.", "");
	ТекстКода1С = СтрЗаменить(ТекстКода1С, "КонецПроцедуры", "//КонецПроцедуры");
...
&НаСервере
Процедура ЗаполнитьМультиязычныйРеквизит(Элемент, ИмяРеквизита, ЛокализуемаяСтрока, КодыЯзыков)
