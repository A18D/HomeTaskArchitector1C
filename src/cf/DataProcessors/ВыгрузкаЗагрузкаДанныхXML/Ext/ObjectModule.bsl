...
Процедура ВыполнитьЗагрузкуДанныхXML(Загружено, ИмяВременногоФайлаСЗамененнымиСсылками, ЧтениеXML)
	...
	Перем ЗаписанноеЗначение, ЗначениеПрочитано, ИмяСобытия, ОписаниеОшибки, ТекстОшибки, ТекстСообщения;
...
Процедура ЗаписатьЗначениеПриНеобходимости(АнализируемоеЗначение, ЗаписьXML, 
	...
	Если СтрокаТаблицы <> Неопределено Тогда
		...
		ДопЗапрос = Новый Запрос("ВЫБРАТЬ * ИЗ "
			+ СтрокаТаблицы.СтрокаДерева.ЭлементОписания.ДляЗапроса
			+ ОбъектМД.Имя
