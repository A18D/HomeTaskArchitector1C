...
Функция ОбъединитьМетоды(ТекстМодуля, ИменаМетодов, ВключаяКомментарии, ВключаяДирективы, 
	...
	Для Индекс = 1 По Количество-1 Цикл
		МетодСправа = ТаблицаМетодов[Индекс];
...
Процедура ПодключаемыеКоманды_ПриЧтенииОсновныхНастроек(Параметры)
	...
	ОбъектыСКомандами = Новый Структура("Печать, ЗаполнениеОбъектов, ВариантыОтчетов, ДополнительныеОтчетыИОбработки");
	...
	КонецЕсли;
	ОбъектыСКомандами.ДополнительныеОтчетыИОбработки = Новый Массив;
	Если Параметры.ЕстьДополнительныеОтчетыИОбработки Тогда
...
Процедура ПодключаемыеКоманды_ЗарегистрироватьПоставщикаКоманд(Параметры, ОбъектМетаданных, Имя)
	Если Параметры.ОбъектыСКомандами[Имя].Найти(ОбъектМетаданных) = Неопределено Тогда
		Параметры.ОбъектыСКомандами[Имя].Добавить(ОбъектМетаданных);
		Параметры.НовыеОбъектыСКомандами[Имя].Добавить(ОбъектМетаданных);
	КонецЕсли;
КонецПроцедуры
	...
	Если ЭтоСписок Тогда
		...
		Если СобытиеТаблицыПриАктивизацииСтроки = Неопределено Тогда
			...
			Пока СтрНайти(НРег(ТекстМодуля), НРег("Процедура " + ИмяСобытия + "(")) > 0 Цикл
				Номер = Номер + 1;
				ИмяСобытия = ИмяТаблицыФормы + "ПриАктивизацииСтроки" + Строка(Номер);
			КонецЦикла;
			Метод = Неопределено;
...
Процедура ДатыЗапретаИзменения_ПриАнализеОбъекта(Параметры, СтрокаОбъекта)
	...
	Для Каждого УзелПредопределенного Из УзлыПредопределенных Цикл
		...
		Параметры.НовыеПредопределенныеРазделыДатЗапрета.Добавить(ТекущийРаздел);
	КонецЦикла;
...
Функция СтрНайтиНеКомментарийИНеСтроку(Строка, ПодстрокаПоиска, Знач НачальнаяПозиция = 1, Знач НомерВхождения = 1, СтрокаМеждуВозвратомКареткиИПодстрокой = "")
	Пока Истина Цикл
