...
Процедура ПриАнализеОбъекта(Параметры, ОписаниеОбъекта)
	
	ПодключаемыеКоманды_ПриАнализеОбъекта(Параметры, ОписаниеОбъекта);
	
КонецПроцедуры

Процедура ПриАнализеФормы(Параметры, ОписаниеОбъекта, ОписаниеФормы)
	...
	Для Каждого КоллекцияОбъектовМетаданных Из КоллекцииОбъектовМетаданных Цикл
		Для Каждого ОбъектМетаданных Из КоллекцияОбъектовМетаданных Цикл
			...
			Если ОбъектМетаданных.ВводитсяНаОсновании.Количество() > 0 Тогда
				Если ВводНаОсновании[ОбъектМетаданных] = Неопределено Тогда
					ВводНаОсновании.Вставить(ОбъектМетаданных, Новый Массив);
				КонецЕсли;
			КонецЕсли;
			
			Для Каждого Основание Из ОбъектМетаданных.ВводитсяНаОсновании Цикл
				ВводимыеОбъекты = ВводНаОсновании[Основание];
				Если ВводимыеОбъекты = Неопределено Тогда
					ВводимыеОбъекты = Новый Массив;
					ВводНаОсновании.Вставить(Основание, ВводимыеОбъекты);
				КонецЕсли;
				
				ВводимыеОбъекты.Добавить(ОбъектМетаданных);
			КонецЦикла;
		КонецЦикла;
	КонецЦикла;
	
	Параметры.Вставить("ВводНаОсновании", ВводНаОсновании);
	
КонецПроцедуры

Процедура ПодключаемыеКоманды_ПриАнализеОбъекта(Параметры, ОписаниеОбъекта)
	
	ВводимыеНаОсновании = Параметры.ВводНаОсновании[ОписаниеОбъекта.Метаданные];
	...
	Если ПроцедураМодуля = Неопределено Тогда
		...
		|//";
		ПроцедураМодуля.Заголовок = "Процедура ДобавитьКомандыСозданияНаОсновании(КомандыСозданияНаОсновании, Параметры) Экспорт";
		ПроцедураМодуля.Подвал = "КонецПроцедуры";
		
		ПроцедураМодуля.Содержимое.Добавить(Символы.Таб);
		Для Каждого ОбъектМетаданных Из ВводимыеНаОсновании Цикл
			ТекстДляВставки = "";
	...
	Если ПроцедураМодуля = Неопределено Тогда
		...
		"Возврат СозданиеНаОсновании.ДобавитьКомандуСозданияНаОсновании(КомандыСозданияНаОсновании, Метаданные.%1);";
		
		Если ЭтоБиблиотечныйОбъект(ОписаниеОбъекта.Метаданные) Тогда
			ШаблонТекстаДляВставки = 
		...
		ИмяОбъекта = ОбщегоНазначения.ИмяБазовогоТипаПоОбъектуМетаданных(ОписаниеОбъекта.Метаданные)
			+ "." + ОписаниеОбъекта.Метаданные.Имя;
		ТекстДляВставки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонТекстаДляВставки, ИмяОбъекта);
		ДобавитьОтступы(ТекстДляВставки);
		
		ПроцедураМодуля.Содержимое.Добавить(Символы.Таб);
		ПроцедураМодуля.Содержимое.Добавить(ТекстДляВставки);
		ПроцедураМодуля.Содержимое.Добавить(Символы.Таб);
	КонецЕсли;
	...
	ПолноеИмяМодуля = Параметры.РабочийКаталог + СтрЗаменить("CommonModules\СозданиеНаОснованииПереопределяемый\Ext\Module.bsl", "\", ПолучитьРазделительПути());
	ОбщийМодуль = ОписаниеМодуля(ПолноеИмяМодуля);
	ПроцедураМодуля = НайтиПроцедуруМодуля(ОбщийМодуль, "ПриОпределенииОбъектовСКомандамиСозданияНаОсновании");
	
	ШаблонДляВставки = "	Объекты.Добавить(Метаданные.%1.%2);";
	
	ТекстДляВставки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(
		ШаблонДляВставки, ОписаниеОбъекта.Родитель.Имя, ОписаниеОбъекта.Имя);
...
Функция НайтиФункциюМодуля(Модуль, ИмяФункции)
	ПрочитатьСтруктуруМодуля(Модуль);
...
Процедура ЗарегистрироватьКоллекциюМетаданных(Параметры, Вид, ПредставлениеСписка, ПредставлениеОбъекта, Ссылочный)
	...
	СтрокаВида = Параметры.ДеревоМетаданных.Строки.Добавить();
	СтрокаВида.Имя       = Вид;
	...
	Для Каждого ОбъектМетаданных Из Коллекция Цикл
		...
		СтрокаОбъекта.ПредставлениеОбъекта = ОбщегоНазначения.ПредставлениеОбъекта(ОбъектМетаданных);
		СтрокаОбъекта.ПредставлениеСписка = ОбщегоНазначения.ПредставлениеСписка(ОбъектМетаданных);
