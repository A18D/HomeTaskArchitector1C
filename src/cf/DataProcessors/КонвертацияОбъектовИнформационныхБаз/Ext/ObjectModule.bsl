...
Процедура ВосстановитьПравилаИзВнутреннегоФормата() Экспорт
	...
	ТаблицаПравилВыгрузки      = СтруктураПравил.ТаблицаПравилВыгрузки; // см. КоллекцияПравилаВыгрузкиДанных
	ТаблицаПравилКонвертации   = СтруктураПравил.ТаблицаПравилКонвертации; // см. КоллекцияПравилаКонвертации
...
Функция ВыборкаДляВыгрузкиСОграничениями(Правило)
	...
	ОбъектПостроительОтчета().Текст = "ВЫБРАТЬ " + СтрокаРазрешения + " Объект.Ссылка КАК Ссылка ИЗ " + ИмяМетаданных + " КАК Объект "+ "{ГДЕ Объект.Ссылка.* КАК " + СтрЗаменить(ИмяМетаданных, ".", "_") + "}";
...
Функция РеквизитыРегистрацииОбъектов()
	...
	Для Каждого ПКО Из КоллекцияПравилаКонвертации() Цикл
...
Процедура ПрочитатьДанныеПоОбмену(ЧтениеСообщения, АнализДанных)
	...
	ЧтениеСообщения = Новый Структура(
		"НомерСообщения, НомерПринятого, Отправитель, ОтправительОбъект, СообщениеБылоПринятоРанее, АнализДанных, ВосстановленаРезервнаяКопия");
...
Процедура ВыполнитьОтложенноеПроведениеДокументов()
	...
	ОбменДаннымиСервер.ВыполнитьОтложенноеПроведениеДокументов(
		ДокументыДляОтложенногоПроведения(), УзелОбменаЗагрузкаДанных);
...
Процедура ВыгрузитьОбщиеДанныеУзлов(Знач НомерОтправленного)
	...
	Для Каждого Свойство Из СвойстваОбщегоУзла Цикл
		...
		Если ТабличныеЧастиОбщегоУзла.Найти(Свойство) <> Неопределено Тогда
			...
			ПКС.ПравилаГруппы = ПравилаКонвертацииСвойств.Скопировать(); // см. КоллекцияПравилаКонвертацииСвойств
...
Процедура ВыполнитьВыгрузкуЗарегистрированныхДанных(ЗаписьСообщения, СтрокаСообщенияОбОшибке, ТаблицаПравилВыгрузкиИспользуемые)
	...
	Попытка
		...
		Пока ВыборкаИзменений.Следующий() Цикл
			...
			Если ЭтоОбменЧерезВнешнееСоединение() Тогда
				// В базе-корреспонденте может не быть этого реквизита обработки.
				Если ОбработкаДляЗагрузкиДанных().Метаданные().Реквизиты.Найти("СчетчикЗагруженныхОбъектовВнешнееСоединение") <> Неопределено Тогда
