...
Перем ПроверяемаяПодсистема, КаталогВыгрузки, СоответствиеТерминов, ДеревоПодсистем, ОтборПоПодсистемам,
	...
	ИмяВидаОбъектаМетаданныхНаЯзыкеКонфигурации, ИмяВидаОбъектаВВыгрузкеФайлов;
...
Функция ИсправляемыеОшибки()
	...
	Список.Добавить("СоставОсновныхРолей",
		НСтр("ru = 'Базовая функциональность.Отличия в составе прав основных ролей от эталонных'"));
...
Процедура ПроверитьСоставОсновныхРолей()
	...
	Если ЭтоДемоБСП() И ИсправлятьОшибку("СоставОсновныхРолей") Тогда
		Для Каждого МакетыРоли Из МакетыОсновныхРолей Цикл
			Если МакетыРоли.Значение.ПраваПоРоли.Шаблон = МакетыРоли.Значение.ПраваПоЭталонуРоли.Шаблон Тогда
				Продолжить;
			КонецЕсли;
			КраткоеОписаниеОшибки = НСтр("ru = 'Не обновлен эталонный макет для проверки роли'");
			ИмяФайлаМакета = КаталогВыгрузки + "Reports\ПроверкаВнедренияБСП\Templates\" + МакетыРоли.Ключ + "\Ext\Template.txt";
	...
	Для Каждого МакетыРоли Из МакетыОсновныхРолей Цикл
		СравнитьРольСЭталоном(МакетыРоли.Ключ, МакетыРоли.Значение);
	КонецЦикла;
	
КонецПроцедуры

Процедура СравнитьРольСЭталоном(ИмяРоли, МакетыРоли)
	
	ЭтоДемоБСП = ЭтоДемоБСП();
	
	Если ЭтоДемоБСП Тогда
		КраткоеОписаниеОшибки         = НСтр("ru = 'Не обновлен эталонный макет для проверки роли'");
		ШаблонОшибки                  = НСтр("ru = 'Изменены права на объект ""%1"" для роли ""%2"".
	...
	Иначе
		КраткоеОписаниеОшибки         = НСтр("ru = 'Права на объект БСП отличаются от поставляемых.'");
	...
	Пока ВыборкаПоОбъекту.Следующий() Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ТекстОшибки, Выборка.Объект, ИмяРоли, Право);
			
			ДобавитьОшибку(Метаданные.Роли[ИмяРоли],
				КраткоеОписаниеОшибки,
				ТекстОшибки);
		...
		Если ФлагИзменения Тогда
			...
			ТекстОшибки = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(ШаблонОшибки , ВыборкаПоОбъекту.Объект, ИмяРоли);
...
Функция ПолучитьПрава(ЧтениеXML, ЭтоЭталон)
	...
	КоличествоОбъектов = ОбъектXDTO.object.Количество();
	СинонимыСтандартныхРеквизитов = СинонимыСтандартныхРеквизитов();
	
	Для Индекс = 1 По КоличествоОбъектов Цикл
		
		ЭлементXDTO = ОбъектXDTO.object[КоличествоОбъектов - Индекс];
		
		ПозицияСтандартнойТабличнойЧасти = СтрНайти(ЭлементXDTO.name, "StandardTabularSection");
		ПозицияСтандартныхРеквизитов = СтрНайти(ЭлементXDTO.name, "StandardAttribute");
		
		Если ПозицияСтандартнойТабличнойЧасти > 0 Тогда
			ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени(Сред(ЭлементXDTO.name, 1, ПозицияСтандартнойТабличнойЧасти - 2));
			ПутьДоРеквизита = Сред(ЭлементXDTO.name, ПозицияСтандартнойТабличнойЧасти);
		...
		ИначеЕсли ПозицияСтандартныхРеквизитов > 0 Тогда
			ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени(Сред(ЭлементXDTO.name, 1, ПозицияСтандартныхРеквизитов - 2));
			ПутьДоРеквизита = Сред(ЭлементXDTO.name, ПозицияСтандартныхРеквизитов);
			ПреобразоватьПутьДоСтандартныхРеквизитов(ПутьДоРеквизита, СинонимыСтандартныхРеквизитов);
			ПолноеИмя = ОбъектМетаданных.ПолноеИмя() + ПутьДоРеквизита;
		ИначеЕсли Лев(ЭлементXDTO.name, 13) = "Configuration" Тогда
			ОбъектМетаданных = Метаданные;
			ПолноеИмя = "Конфигурация.БиблиотекаСтандартныхПодсистем";
			ЭлементXDTO.name = "Configuration.БиблиотекаСтандартныхПодсистем";
		Иначе
			ОбъектМетаданных = Метаданные.НайтиПоПолномуИмени(ЭлементXDTO.name);
			Если ОбъектМетаданных = Неопределено Тогда
				ПолноеИмя = ЭлементXDTO.name;
			Иначе
		...
		Если НЕ ЭтоЭталон
			...
			ОбъектXDTO.object.Удалить(КоличествоОбъектов - Индекс);
			Продолжить;
		КонецЕсли;
		
		Если ТипЗнч(ЭлементXDTO.right) = Тип("ОбъектXDTO") Тогда
			НоваяСтрока = ТаблицаПрав.Добавить();
			НоваяСтрока.Объект   = ПолноеИмя;
			НоваяСтрока.Право    = ЭлементXDTO.right.name;
			НоваяСтрока.Значение = ЭлементXDTO.right.value;
		Иначе
			Для Каждого ПравоXDTO Из ЭлементXDTO.right Цикл
				НоваяСтрока = ТаблицаПрав.Добавить();
				НоваяСтрока.Объект   = ПолноеИмя;
				НоваяСтрока.Право    = ПравоXDTO.name;
				НоваяСтрока.Значение = ПравоXDTO.value;
			КонецЦикла;
		КонецЕсли;
...
Функция МакетыРоли(ИмяРоли)
	...
	ФайлРолиПолныеПрава = КаталогВыгрузки + "Roles\" + ИмяРоли + "\Ext\Rights.xml";
	
	// Открываем чтение свойств роли конфигурации.
	ЧтениеXML = Новый ЧтениеXML;
	ЧтениеXML.ОткрытьФайл(ФайлРолиПолныеПрава);

	СтруктураВозврата.ПраваПоРоли = ПолучитьПрава(ЧтениеXML, Ложь);
	...
	ТекстовыйДокументПолныеПрава.Записать(ПотокВПамяти);
	ПотокВПамяти.Перейти(0, ПозицияВПотоке.Начало);
	
	ЧтениеXML = Новый ЧтениеXML;
	ЧтениеXML.ОткрытьПоток(ПотокВПамяти);
	
	СтруктураВозврата.ПраваПоЭталонуРоли = ПолучитьПрава(ЧтениеXML, Истина);
...
Процедура ПроверитьНаличиеВставкиКодаДляМассива(ПараметрыПроверкиВходящие)
	...
	Если ПроверитьФормуОбъекта Или ПроверитьФормуСписка Тогда
