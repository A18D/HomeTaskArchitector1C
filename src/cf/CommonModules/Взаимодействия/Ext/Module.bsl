...
Функция НайтиКонтактыСАдресами(СтрокаПоиска) Экспорт
	...
	Для каждого ОписаниеКонтакта Из ОписанияКонтактов Цикл
		...
		|ВЫБРАТЬ ПЕРВЫЕ 20
		|	СправочникКонтакт.Ссылка КАК Контакт,
		|	СправочникКонтакт." + ОписаниеКонтакта.ИмяРеквизитаПредставлениеКонтакта + " КАК Наименование,
		|	" + ?(ОписаниеКонтакта.ЕстьВладелец, "ПРЕДСТАВЛЕНИЕ(СправочникКонтакт.Владелец)", """""") + " КАК НаименованиеВладельца,
		|	ТаблицаКонтактнаяИнформация.АдресЭП КАК Представление
		|ИЗ
		|	Справочник." + ОписаниеКонтакта.Имя + " КАК СправочникКонтакт
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Справочник." + ОписаниеКонтакта.Имя + ".КонтактнаяИнформация КАК ТаблицаКонтактнаяИнформация
		|		ПО (ТаблицаКонтактнаяИнформация.Ссылка = СправочникКонтакт.Ссылка)
		...
		|ГДЕ
		|	НЕ СправочникКонтакт.ПометкаУдаления И (ТаблицаКонтактнаяИнформация.АдресЭП ПОДОБНО &ВведеннаяСтрока
		|			ИЛИ ТаблицаКонтактнаяИнформация.ДоменноеИмяСервера ПОДОБНО &ВведеннаяСтрока
		|			ИЛИ ТаблицаКонтактнаяИнформация.Представление ПОДОБНО &ВведеннаяСтрока 
		|" + УсловиеПолейВводаПоСтроке + ") 
		|";
...
Функция КонтактыПоДоменномуАдресу(Знач ИмяДомена)
	...
	Для каждого ОписаниеКонтакта Из ОписанияКонтактов Цикл
		...
		Если ОписаниеКонтакта.ИскатьПоДомену Тогда
			...
			|	ТаблицаКонтактнаяИнформация.Ссылка." + ОписаниеКонтакта.ИмяРеквизитаПредставлениеКонтакта + " КАК Наименование,
			|	ТаблицаКонтактнаяИнформация.АдресЭП КАК Представление,
			|	" + ?(ОписаниеКонтакта.ЕстьВладелец,"ТаблицаКонтактнаяИнформация.Ссылка.Владелец.Наименование ","""""") + ?(ПерваяТаблица," КАК НаименованиеВладельца ","") +" 
			|ИЗ
			|	Справочник." + ОписаниеКонтакта.Имя + ".КонтактнаяИнформация КАК ТаблицаКонтактнаяИнформация
			|ГДЕ
...
// Параметры:
//  Форма - ФормаКлиентскогоПриложения - форма документа взаимодействия.
//
...
//  ТипКонтактнойИнформации     - Перечисления.ТипыКонтактнойИнформации - если указано, 
...
Функция АдресаЭлектроннойПочтыКонтактов(Контакты, ГруппаПолучателей = "") Экспорт
	...
	Для каждого ОписаниеКонтакта Из ОписанияКонтактов Цикл
		...
		|	ТаблицаКонтакт.Ссылка
		|ИЗ
		|	Справочник." + ОписаниеКонтакта.Имя + " КАК ТаблицаКонтакт
		|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник." + ОписаниеКонтакта.Имя + ".КонтактнаяИнформация КАК ТаблицаКонтактнаяИнформация
		|		ПО (ТаблицаКонтактнаяИнформация.Ссылка = ТаблицаКонтакт.Ссылка)
		|			И (ТаблицаКонтактнаяИнформация.Тип = ЗНАЧЕНИЕ(Перечисление.ТипыКонтактнойИнформации.АдресЭлектроннойПочты))
		|ГДЕ
		|	НЕ ТаблицаКонтакт.ПометкаУдаления
		|	И ТаблицаКонтакт.Ссылка В(&МассивКонтактов) " + ?(ОписаниеКонтакта.Иерархический," И НЕ ТаблицаКонтакт.ЭтоГруппа","");
...
Функция ВыполнитьОтправкуПисьма(Объект, Соединение = Неопределено, ПараметрыПисьма = Неопределено, ПротоколПочты = "") Экспорт
	...
	ИдентификаторПисьма = РаботаСПочтовымиСообщениями.ОтправитьПочтовоеСообщение(Объект.УчетнаяЗапись, ПараметрыПисьма); // АПК:223
...
Процедура УстановитьУсловноеОформлениеВзаимодействия(Форма) Экспорт
	...
	ПолеЭлемента.Поле = Новый ПолеКомпоновкиДанных(Форма.Элементы.ПредставлениеКонтакта.Имя);
...
Процедура ДобавитьСтраницыФормыПодбораКонтактов(Форма) Экспорт
	...
	Для каждого ОписаниеКонтакта Из ОписанияКонтактов Цикл
		...
		ДобавляемыеРеквизиты.Добавить(
			Новый РеквизитФормы("Список_" + ОписаниеКонтакта.Имя, ОписаниеТипаДинамическийСписок));
	КонецЦикла;
	...
	Для каждого ОписаниеКонтакта Из ОписанияКонтактов Цикл
		...
		ЕстьКолонкаАдрес = УстановитьТекстЗапросаКонтакта(Форма["Список_" + ОписаниеКонтакта.Имя], ОписаниеКонтакта);
	...
	Для каждого ОписаниеКонтакта Из ОписанияКонтактов Цикл
		...
		Если НЕ ОписаниеКонтакта.ЕстьВладелец  Тогда
			...
			ЭлементСтраница = Форма.Элементы.Добавить(
				"Страница_" + ОписаниеКонтакта.Имя,Тип("ГруппаФормы"), Форма.Элементы.СтраницыСписки);
		...
		ЭлементТаблица = Форма.Элементы.Добавить("Таблица_" + ОписаниеКонтакта.Имя,	Тип("ТаблицаФормы"),
		...
		Если ОписаниеКонтакта.ЕстьВладелец Тогда
			Форма.Элементы["Таблица_" + ОписаниеКонтакта.ИмяВладельца].УстановитьДействие(
			...
			Форма.Элементы["Таблица_" + ОписаниеКонтакта.ИмяВладельца].Высота = 5;
		Иначе
