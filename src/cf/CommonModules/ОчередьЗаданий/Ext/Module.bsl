...
//	 * Идентификатор - СправочникСсылка.ОчередьЗаданий, СправочникСсылка.ОчередьЗаданийОбластейДанных
//	 ...
Функция ПолучитьЗадания(Знач Отбор) Экспорт
	...
	Для Каждого СправочникЗаданий Из СправочникиЗаданий Цикл
		...
		Если Не ПолучатьРазделенные Тогда
			...
			Если РаботаВМоделиСервисаПовтИсп.ЭтоРазделеннаяКонфигурация() 
			   И РаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(ИмяСправочника, РазделительЗаданийОбластейДанных) Тогда
				Продолжить;
			КонецЕсли;
		...
		Если Не ПолучатьНеразделенные Тогда
			...
			Если Не РаботаВМоделиСервисаПовтИсп.ЭтоРазделеннаяКонфигурация() 
			 Или Не РаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(ИмяСправочника, РазделительЗаданийОбластейДанных) Тогда
				Продолжить;
			КонецЕсли;
		...
		Если ТаблицаУсловий.Количество() > 0 Тогда
			...
			Для Каждого Условие Из ТаблицаУсловий Цикл
				...
				Если НЕ ПустаяСтрока(СтрокаУсловий) Тогда
					СтрокаУсловий = СтрокаУсловий + СтрШаблон("%1%2%3 ", Символы.ПС, Символы.Таб, "И");
				КонецЕсли;
				
				СтрокаУсловий = СтрокаУсловий + СтрШаблон("%1.%2 %3 (&%4)", "Очередь", Условие.Поле, ВидыСравнения.Получить(Условие.ВидСравнения), Условие.Параметр);
		...
		Если РаботаВМоделиСервисаПовтИсп.ЭтоРазделеннаяКонфигурация() 
		   И РаботаВМоделиСервиса.ЭтоРазделенныйОбъектМетаданных(ИмяСправочника, РаботаВМоделиСервиса.РазделительВспомогательныхДанных()) Тогда
			...
			ТекстЗапроса = ТекстЗапроса + СтрЗаменить(СтрЗаменить(
				...
				|ИЗ
				|	&Очередь КАК Очередь ЛЕВОЕ СОЕДИНЕНИЕ Константа.ЧасовойПоясОбластиДанных КАК ЧасовыеПояса
				|		ПО Очередь.ОбластьДанныхВспомогательныеДанные = ЧасовыеПояса.ОбластьДанныхВспомогательныеДанные",
				"&Очередь", ИмяСправочника), "&ПоляВыборки", ПоляВыборки);
		...
		Иначе
			...
			ТекстЗапроса = ТекстЗапроса + СтрЗаменить(СтрЗаменить(
				...
				|	"""" КАК ЧасовойПояс
				|ИЗ
				|	&Очередь КАК Очередь", "&Очередь", ИмяСправочника), "&ПоляВыборки", ПоляВыборки);
			
		КонецЕсли;
		
		Если Не ПустаяСтрока(СтрокаУсловий) Тогда
			
			ТекстЗапроса = ТекстЗапроса + СтрШаблон("%1%2%1%3%4",Символы.ПС, "ГДЕ", Символы.Таб, СтрокаУсловий);
	...
	КолонкаРасписание = Результат.Колонки.Расписание; // КолонкаТаблицыЗначений
	КолонкаРасписание.Имя = "РасписаниеХранилище";
	КолонкаПараметры = Результат.Колонки.Параметры; // КолонкаТаблицыЗначений
	КолонкаПараметры.Имя = "ПараметрыХранилище";
...
Функция ДобавитьЗадание(ПараметрыЗадания) Экспорт
	...
	Если ПараметрыЗадания.Свойство("Ключ") И ЗначениеЗаполнено(ПараметрыЗадания.Ключ) Тогда
		...
		Если ПараметрыЗадания.ОбластьДанных <> -1 Тогда
...
Функция ПолучитьТекстИсключенияДублированиеЗаданийСОдинаковымКлючом() Экспорт
	...
	Возврат НСтр("ru = 'Дублирование заданий с одинаковым значением поля ''Ключ'' не допустимо.'");
