...
//  Используется для запуска выгрузки и загрузки данных в конфигурациях, не
//  содержащих БСП.
//
...
Процедура ЗагрузитьДанныеИзКаталога(Знач КаталогЗагрузки, Знач ПараметрыЗагрузки) Экспорт
	...
	Если ЕстьУстановленныеРасширения Тогда
		...
		МассивПараметров.Добавить(КаталогЗагрузки);
...
//
Процедура ВыполнитьЗагрузкуДанныхИнформационнойБазыВФоне(Знач КаталогЗагрузки, Знач ПараметрыЗагрузки) Экспорт 
...
Функция XMLТипСсылки(Знач Значение) Экспорт
	...
	Если ТипЗнч(Значение) = Тип("ОбъектМетаданных") Тогда
		...
		МенеджерОбъекта = ИнтеграцияПодсистемБТС.МенеджерОбъектаПоПолномуИмени(ОбъектМетаданных.ПолноеИмя());
...
Функция ПрочитатьИнформациюОВыгрузке(Контейнер)
	...
	ИмяФайла = Контейнер.ПолучитьФайлИзКаталога(DumpInfo());
	
	Возврат ПрочитатьОбъектXDTOИзФайла(
		ИмяФайла, ФабрикаXDTO.Тип("http://www.1c.ru/1cFresh/Data/Dump/1.0.2.1", "DumpInfo"));
...
//
// Параметры:
//	Типы - ФиксированныйМассив Из Метаданные - массив типов.
//
// Возвращаемое значение:
//	ФабрикаXDTO - фабрика.
//
