...
//  ПравилаОбработкиДанных - ТаблицаЗначений - таблица значений, в которую будут добавлены правила.
//
...
Процедура ПОД_Документ_КорректировкаОстатковТоваров_Отправка_ПриОбработке(ДанныеИБ, ИспользованиеПКО, КомпонентыОбмена)
	...
	Для Каждого СтрокаДвиженияДокумента Из ДвиженияДокумента Цикл
		...
		Если СтрокиОбъекты.Количество() = 0 Тогда
			...
			СтрокаОбъекты.Товары = Новый ТаблицаЗначений;
			СтрокаОбъекты.Товары.Колонки.Добавить("Номенклатура", Новый ОписаниеТипов("СправочникСсылка._ДемоНоменклатура"));
			СтрокаОбъекты.Товары.Колонки.Добавить("Количество",   Новый ОписаниеТипов("Число"));
			СтрокаОбъекты.Товары.Колонки.Добавить("Цена",         Новый ОписаниеТипов("Число"));
			СтрокаОбъекты.Товары.Колонки.Добавить("Сумма",        Новый ОписаниеТипов("Число"));
			СтрокаОбъекты.Товары.Колонки.Добавить("ТипЗапасов",   Новый ОписаниеТипов("Строка"));
		Иначе
			СтрокаОбъекты = СтрокиОбъекты[0];
		КонецЕсли;
		
		СтрокаТовары = СтрокаОбъекты.Товары.Добавить();
